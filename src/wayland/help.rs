...
fn draw_text(canvas: &mut Canvas, font: &Font, text: &str, x: i32, y: i32, color: [u8; 4]) {
...
fn draw_border(canvas: &mut Canvas, x: i32, y: i32, w: u32, h: u32) {
    canvas.draw_rect(x, y, w, 1, BORDER_COLOR);
    canvas.draw_rect(x, y + h as i32 - 1, w, 1, BORDER_COLOR);
    canvas.draw_rect(x, y, 1, h, BORDER_COLOR);
    canvas.draw_rect(x + w as i32 - 1, y, 1, h, BORDER_COLOR);
}

pub(super) fn render_help(canvas: &mut Canvas) -> Rect {
    let font_regular =
    ...
    draw_border(canvas, panel_x, panel_y, panel_w, panel_h);
    ...
    draw_text(canvas, &font_bold, HELP_TITLE, title_x, row_y, TITLE_COLOR);
